...
static int initial_white_pieces[16][2] = {
    { W_PAWN,   A2 },
    { W_PAWN,   B2 },
    { W_PAWN,   C2 },
    { W_PAWN,   D2 },
...
static int initial_black_pieces[16][2] = {
    { B_PAWN,   A7 },
    { B_PAWN,   B7 },
    { B_PAWN,   C7 },
    { B_PAWN,   D7 },
...
void init_new_game(gamestate_t *state)
{
    ...
    memcpy(state->black_pieces, initial_white_pieces, sizeof(int)*16*2);
}
...
piece_t get_piece_at(gamestate_t *state, square_t square)
{
    for (int i = 0; i < 16; i++) {
        if (state->white_pieces[i][LOCATION] == square) {
            return state->white_pieces[i][LOCATION];
        }
        if (state->black_pieces[i][LOCATION] == square) {
            return state->black_pieces[i][LOCATION];
        }
    }
    return PIECE_NONE;
}
